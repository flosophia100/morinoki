...
import { META as DESIGN_META, DEFAULTS as DESIGN_DEFAULTS,
         ...
         PALETTE_OPTIONS, WEATHER_OVERRIDE_OPTIONS } from './designconfig.js';
  ...
  return `
    ...
    <div class="ip-block">
      ...
      <div class="color-row" id="mn-color">
        ${colorList.map(c => `<button data-color="${c}" style="box-shadow:inset 0 0 0 100px ${c}" class="${c===node.color?'on':''}"></button>`).join('')}
      </div>
...
function adminAmbienceTab(state) {
  ...
  return `
    <div class="ip-block">
      ...
      <label class="design-label" style="margin-top:0.5rem">天気エフェクト</label>
      <select data-ambience-key="weatherOverride" class="admin-select">
  ...
  return `
    ...
    <div class="ip-block">
      ...
      <div class="color-row" id="nf-color">
        ${palette.map(c => `<button data-color="${c}" style="box-shadow:inset 0 0 0 100px ${c}" class="${c===node.color?'on':''}"></button>`).join('')}
      </div>
      <label class="mini-label">説明(任意・300字)</label>
